...
class ZoneQueryManager:
	def __init__(self):
		self.Q_PROJECT_INSTANCES = """ "select project_id, count(state) from instances where state=1 or state=5 and deleted=0 group by project_id;" """
		...
		self.Q_PROJECT_CPUS      = """ "select project_id, sum(vcpus) from instances where state=1 or state=5 and deleted=0 group by project_id;" """

...
print("mkdir -p %(mount_point)s" % locals())
...
if fs.f_bfree < fs.f_blocks / 2:
    ...
    print("mke2fs -q -t ext3 %(device)s" % locals())

    #print("\n***** Mounting volume to %(mount_point)s *****" % locals())
    print("mount %(device)s %(mount_point)s" % locals())
...
print("euca-bundle-vol --no-inherit %(kernel_opt)s %(ramdisk_opt)s -d %(mount_point)s -r x86_64 -p %(image_name)s -s %(disk_size_in_MBs)s -e %(dirs_to_exclude)s" % locals())

#print("\n***** Uploading bundle *****")
print("euca-upload-bundle -b %(bucket_name)s -m %(mount_point)s/%(image_name)s.manifest.xml" % locals())

#print("\n***** Registering bundle *****")
print("euca-register %(bucket_name)s/%(image_name)s.manifest.xml" % locals())
